...
__global__ void printCoverTable(const int * vCoverTable, const int * vCoverTableLinks, 
    const int * vCoverTableLens, const int * vLastAppearLevel, int nObjs);
    ...
    while (levelCount >= 0) {
        ...
        for (int i = 0; i < innerExpandTimes; i++) {
            ...
            while (true) {
                ...
                // note, updating min values does not affect the existing node ids inserted in previous batches,
                // since they have already been masked out (=2) in this batch
                reconstructHashTable.insert_batch_update_min_masked(vStepNodeKeys, vStepNodeValues, vStepNodeMask, thisReadyCovers);
                ...
                // for new nodes, insert the associated levels into level ht. 
                levelHashTable.insert_batch_update_min_masked(vStepNodeValues, vStepNodeLevels, vStepNodeMask, thisReadyCovers);
...
__global__ void printCoverTable(const int * vCoverTable, const int * vCoverTableLinks, 
    ...
    if (idx == 0) {
        for (int nodeId = 1; nodeId < nObjs; nodeId++) {
            ...
            printf("  Cover of %d (lastAppearLevel = %d): ", nodeId, vLastAppearLevel[nodeId]);
            int currRowIdx = nodeId;
            int columnPtr = 0;
            int currLit, currId;
            for (int i = 0; i < length; i++) {
